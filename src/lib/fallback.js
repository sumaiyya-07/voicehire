...
export function generateLocalQuestions({ jobRole, interviewType, difficulty, numQuestions, topic }) {
    ...
    if (selected.length > 0) {
        selected[0] = selected[0].replace(/this role|this field|your experience/gi, `the ${jobRole} role`);
    }
    return selected;
}
...
export function evaluateAnswerLocally(questionText, answerText, difficulty) {
    ...
    const stopWords = new Set([
        ...
        'by', 'from', 'as', 'into', 'about', 'between', 'through', 'during', 'before', 'after', 'above', 'below',
        'and', 'but', 'or', 'not', 'no', 'nor', 'so', 'yet', 'both', 'either', 'neither', 'each', 'every', 'all', 'any',
        'few', 'more', 'most', 'other', 'some', 'such', 'than', 'too', 'very', 'just', 'also', 'now', 'here', 'there',
        'when', 'where', 'why', 'how', 'what', 'which', 'who', 'whom', 'this', 'that', 'these', 'those', 'it', 'its',
        'i', 'me', 'my', 'myself', 'we', 'our', 'you', 'your', 'he', 'she', 'they', 'them', 'tell', 'describe',
        'explain', 'give', 'example', 'time', 'situation',
    ]);
    ...
    if (score < 20) {
        ...
        const opts = ['You addressed the question but could provide more depth and specific examples.', 'Your answer shows basic understanding of the topic.'];
        positive = opts[Math.floor(Math.random() * opts.length)];
        ...
        const opts = ['Strong answer with relevant details and good structure.', 'You demonstrated clear understanding and provided specifics.'];
        positive = opts[Math.floor(Math.random() * opts.length)];
...
export function generateReportLocally(interview, qas) {
    ...
    const avgScore = scores.length ? Math.round(scores.reduce((a, b) => a + b, 0) / scores.length) : 60;
    const grade = avgScore >= 85 ? 'Excellent' : avgScore >= 70 ? 'Good' : avgScore >= 55 ? 'Average' : avgScore >= 40 ? 'Needs Improvement' : 'Poor';
    ...
    return {
        ...
        strengths: ['You completed the interview and attempted all questions.', 'Your responses showed clarity of thought.', 'You demonstrated relevant domain knowledge.'],
        improvements: ['Practice structuring answers using the STAR method.', 'Include more specific, quantified examples from your experience.', `Research the ${interview.job_role} role more deeply before interviews.`],
